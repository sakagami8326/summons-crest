...
- **選択ドロー**(v0.61): 手番開始時に山札から2枚見て1枚を手札へ・1枚を捨て札へ(`startPickDraw(r,p,n,after)`/pending `pick_draw`/候補は本人のみ公開)。**制限時間なし**(v0.63でタイムアウト廃止)。**祠は3枚から1枚選ぶ選択ドロー**(v0.72・完了後は`after:'end'`で手番終了)。戦闘勝利・ひらめきは従来どおり直接ドロー。
