...
- 現在バージョン: **v0.57**(server.jsの`VERSION`とboard.html左下の`board X.XX`表記)
...
4. **UI設計思想**: マス(土地)を選ぶ操作は、スマホ画面にミニマップを表示してタップさせる方式に統一する(強化・売却・スペル対象などすべて)。テキストのボタン列にしない。
...
詳細は`docs/manual_v0.56.md`と各仕様書(spec_spells_v0.53.md / spec_creatures_v0.57.md)。実装済みの重要ポイント:

- **勝利**: 総資産8000G以上で城を通過(資産=所持金+地価+称号500G+秘宝600G)。7000Gでラストスパート。
- **地価**=100G×Lv倍率(1/2.5/5/8)×連鎖倍率(1.0/1.4/1.8/2.2/2.6)。**通行料**=地価×25%×rate(清流+0.2、満ち潮+0.5の加算式)。
- **破産**: 所持金マイナス→強制売却(70%)→それでも負なら脱落。破産者にpendingを出さないこと。
- **刻印**: 門通過で取得、城通過で消費して一周ボーナス(+200G+地価10%+無料ドラフト+自領地負傷-10)。周回p.lapは刻印に関わらず+1。
