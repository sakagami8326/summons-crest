...
ok(/const VERSION = '1\.58'/.test(serverSource) && require('./package.json').version === '1.58.0', 'v1.53 feedback remains covered by v1.58');
...
(async () => {
  ...
  try {
    ...
    ok(received[0].token === 'test-only-shared-token' && received[0].version === '1.58' && received[0].sendId, 'server adds token, current version, and submission ID');
