...
ok(/const VERSION = '1\.55'/.test(serverSource) && require('./package.json').version === '1.55.0', 'v1.53 feedback remains covered by v1.55');
...
(async () => {
  ...
  try {
    ...
    ok(received[0].token === 'test-only-shared-token' && received[0].version === '1.55' && received[0].sendId, 'server adds token, current version, and submission ID');
