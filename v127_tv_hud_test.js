...
ok(/#hud \{ top:40px !important; left:30px !important; width:456px !important; \}/.test(board), 'HUD uses the wider safe-area layout');
ok(/\.plate \{[^}]*overflow:hidden/s.test(board), 'HUD clips artwork to the plate');
ok(/\.hudPortrait \{[^}]*left:0; bottom:0; width:146px; height:114px/s.test(board), 'HUD portrait stays inside the plate');
...
ok(/margin-left:126px !important/.test(board) && /margin-left:134px !important/.test(board), 'HUD text clears the portrait area');
ok(/@media \(max-width:1400px\), \(max-height:800px\)/.test(board), 'HUD has a compact-TV fallback');
