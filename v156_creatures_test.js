...
eq(G.VERSION, '1.62', 'server version');
...
eq([G.MARKET_POOL.length, deck.length], [41,150], 'market and common deck totals');
for (const id of ['wakatama','emeri','valk']) {
  eq([count(deck,id), count(shop,id), count(deck,id + '_f')], [2,2,0], `${id} copy rules`);
}
eq(G.publicCardCatalog().counts, { total:76, creatures:46, evolutions:39, spells:25, weapons:5 }, 'catalog counts');
...
ok(cardsPage.includes('data-total>76') && cardsPage.includes('data-evolutions>39'), 'cards page fallback counts updated');
ok(rulesPage.includes('土領地に水属性クリーチャーがいても1つ') && rulesPage.includes('回復と「恵みの水脈」'), 'public rules explain corrected mechanics');
ok(manual.includes('カード一覧(クリーチャー46種)') && manual.includes('v1.56 回復報酬・土領地連動クリーチャー'), 'manual updated');
ok(spec.includes('共通山札150枚') && spec.includes('配置クリーチャーの属性は参照しない'), 'spec updated');
